---
website: "Câmara Municipal de Águeda (sítio Web institucional)"          # Entre as aspas escreve o nome do website
date: "25/02/2026"                    # Entre as aspas escreve a data de criação do 1º relatório. Os restantes estão no histórico
uri: "https://www.cm-agueda.pt"   # Entre as aspas escreve o domínio do website
owner: "Câmara Municipal de Águeda"         # Entre as aspas escrever o nome do owner do website
seal: "Ouro"                          # Entre as aspas escreve Bronze, Prata ou Ouro
validity: "24/09/2026 a 24/09/2027" # Entre as aspas escreve data de início e data de fim no formato 31/12/1999 a 31/12/2000
status: "Concluído" # Entre as aspas escreve uma das seguintes opções: "Auditoria a decorrer", "A aguardar correções da entidade", "Concluído" 
---

# {{ page.website }}

- Data de criação: {{ page.date }}
- URL: {{ page.uri }}
- Propriedade: {{ page.owner }}
- Candidatura: {{ page.seal }}

## Relatório de Auditoria

Consulte aqui a última atualização: [Relatório do {{ page.website }}](report.html)

<details>
  <summary>Histórico de atualizações</summary>
  <ul aria-label="lista de relatórios já efetuados">
    <li><a href="24092026_report.html">(24/09/2026). Relatório do {{ page.website }}</a></li>
    <li><a href="ddmmaaaa_report.html">(dd/mm/aaaa). Relatório do {{ page.website }}</a></li>
    <li><a href="/report_012/25022026_report.html">(25/02/2026). Relatório do {{ page.website }}</a></li>
  </ul>
</details>

## Declaração de Acessibilidade analisada

- [https://www.cm-agueda.pt/ficha-tecnica/acessibilidade](https://www.cm-agueda.pt/ficha-tecnica/acessibilidade)
